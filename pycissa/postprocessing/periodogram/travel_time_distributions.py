...
def gamma_function(x,tau0,exponent,amplitude):
...
def fit_gamma(my_freq,my_psd):
    try:popt, pcov = curve_fit(gamma_function, my_freq, [x/1 for x in my_psd],method = 'trf', x_scale = [1, 1, 1])
    ...
    #robust fitting
    x0 = np.ones(3)
    res_robust = least_squares(gamme_function_robust, x0, loss='huber', f_scale=0.01, args=(my_freq, [x/max(my_psd) for x in my_psd]))
    r2_robust = r2_score(my_psd, gamma_function(my_freq, res_robust.x[0], res_robust.x[1],res_robust.x[2]))
